...
| [architecture/auth.md](architecture/auth.md) | The auth context: token strategy (one secret per purpose), the login and OAuth-callback decision tables, the pre-hijacking defence, cookie scoping, global guards, and known gaps. | Anyone touching login, tokens, OAuth, or the guards. |
...
| [architecture/decisions/0006-stateless-tokens-no-storage.md](architecture/decisions/0006-stateless-tokens-no-storage.md) | Why every token is a stateless JWT with no server-side storage, and the revocation gap that accepts. | Anyone questioning token design or adding revocation. |
| [architecture/decisions/0007-oauth-provider-abstraction.md](architecture/decisions/0007-oauth-provider-abstraction.md) | Why `IOAuthProvider` exists with one implementation, and why the decision table keys on `providerUserId` rather than email. | Anyone adding an OAuth provider or touching the callback logic. |
